...
public class AdminController {
  ...
  @PostMapping("/addBook")
  public String addBook(@Valid @ModelAttribute("bookDto") BookDto bookDto, BindingResult result) {
    if (bookDto.getImageFile().isEmpty()) {
      result.addError(new FieldError("bookDto", "imageFile", "Изображение не было загружено"));
    }
    if (result.hasErrors()) {
      return "add-book";
    }
  ...
  public String showEditBookPage(@PathVariable("bookId") Long bookId, Model model) {
  ...
  public String deleteBook(
      @PathVariable("bookId") Long bookId, RedirectAttributes redirectAttributes) {
    try {
      booksService.deleteBook(bookId);
      redirectAttributes.addFlashAttribute("successMessage", "Удалено");
    } catch (DataIntegrityViolationException e) {
      redirectAttributes.addFlashAttribute(
          "errorMessage", "Невозможно удалить, так как находится в корзине");
    }
    return "redirect:/books";
  }
}
